...
export default function StoryNewScreen() {
  ...
  const { user, myStories, addStory, updateStory } = useApp();

  // Если storyId передан — режим редактирования
  const editEntry = storyId ? myStories.find((s) => s.id === storyId) : undefined;
  const isEdit    = !!editEntry;

  const [mediaUri, setMediaUri]       = useState<string | null>(editEntry?.mediaUri ?? null);
  ...
  function handlePublish() {
    Keyboard.dismiss();
    if (isEdit && storyId) {
      updateStory(storyId, text, mediaUri ?? undefined);
    } else {
      addStory(text, mediaUri ?? undefined);
    }
    router.back();
  }
  ...
  const Header = (
    <View style={[styles.header, { paddingTop: insets.top + 10 }]}>
      ...
      <View style={styles.authorRow}>
        ...
        <Text style={styles.authorName}>{isEdit ? "Редактирование" : "Новая сториз"}</Text>
      </View>
      <TouchableOpacity
    ...
    return (
      <LinearGradient colors={["#111009", "#1C1408"]} style={styles.screen}>
        ...
        <View style={styles.pickArea}>
          ...
          <Text style={styles.pickSub}>Медиа — основа сториз</Text>
  ...
  return (
    <TouchableWithoutFeedback onPress={Keyboard.dismiss}>
      <View style={styles.screen}>
        ...
        <View style={[StyleSheet.absoluteFillObject, { backgroundColor: "rgba(0,0,0,0.26)" }]} />
        ...
        {mediaUri !== "text_only" && (
          <View style={styles.changeRow}>
            <TouchableOpacity style={styles.changeBtn} onPress={() => setMediaUri(null)} activeOpacity={0.8}>
              ...
              <Text style={styles.changeBtnText}>Изменить медиа</Text>
            </TouchableOpacity>
          </View>
        )}
        ...
        >
          <TouchableWithoutFeedback onPress={() => { inputRef.current?.focus(); setEditingText(true); }}>
            <View style={[styles.textBox, editingText && styles.textBoxActive]}>
              {editingText || text.length > 0 ? (
                ...
                <View style={styles.addHint}>
                  <Feather name="type" size={18} color="rgba(255,255,255,0.65)" />
                  <Text style={styles.addHintText}>Нажмите, чтобы добавить текст</Text>
                </View>
              )}
              {(editingText || text.length > 0) && (
        ...
        {!editingText && (
          <View style={[styles.footer, { paddingBottom: insets.bottom + 20 }]}>
            ...
            >
              <LinearGradient
                colors={canPublish ? [GOLD, "#7A4A1E"] : ["rgba(255,255,255,0.1)", "rgba(255,255,255,0.06)"]}
                style={styles.bigBtnGrad}
              >
                <Feather name={isEdit ? "check" : "send"} size={18} color={canPublish ? "#FFF" : "rgba(255,255,255,0.3)"} />
                <Text style={[styles.bigBtnText, !canPublish && styles.bigBtnTextOff]}>
                  {isEdit ? "Сохранить изменения" : "Опубликовать сториз"}
                </Text>
              </LinearGradient>
            </TouchableOpacity>
          </View>
...
const styles = StyleSheet.create({
  ...
  header: { flexDirection: "row", alignItems: "center", paddingHorizontal: 14, paddingBottom: 10, gap: 10, zIndex: 20 },
  ...
  publishBtn: { backgroundColor: GOLD, paddingHorizontal: 16, paddingVertical: 8, borderRadius: 20 },
  ...
  pickSub: { color: "rgba(255,255,255,0.5)", fontSize: 14, textAlign: "center", marginBottom: 20 },
  ...
  changeRow: { paddingHorizontal: 16, paddingBottom: 8, zIndex: 10 },
  ...
  textBox: { minWidth: 200, maxWidth: 300, minHeight: 60, alignItems: "center", justifyContent: "center", padding: 16, borderRadius: 16 },
  textBoxActive: { backgroundColor: "rgba(0,0,0,0.45)", borderWidth: 1, borderColor: "rgba(200,160,100,0.4)" },
  textInput: { color: "#FFF", fontSize: 20, fontFamily: "Inter_600SemiBold", textAlign: "center", minWidth: 200, maxWidth: 280, minHeight: 40 },
  charCount: { color: "rgba(255,255,255,0.3)", fontSize: 11, marginTop: 6, alignSelf: "flex-end" },
  addHint: { alignItems: "center", gap: 8 },
  addHintText: { color: "rgba(255,255,255,0.6)", fontSize: 14 },
